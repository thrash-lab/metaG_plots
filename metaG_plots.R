...
##############################################################
# Read in the data 
df <- read.csv("~/Google Drive File Stream/My Drive/Thrash_Lab_Projects/OM252/Analyses/metaG_ecology/OM252_all_metaG_data.csv", header=T)  
#df <- read.csv("~/Google Drive File Stream/My Drive/Thrash_Lab_Projects/OM252/Analyses/metaG_ecology/OM252_all_coastal_metaG_data.csv", header=T)  
...
ggsave("map_RPKM_by_genome.pdf", dpi = 300, width = 11.5, height = 8)

##############################################################
# Map abundances for specific regions
#sub_data <- datamelt[datamelt$db=='FRANCIS',]
...
#ggsave("SFBay_map_RPKM_by_genome.pdf", dpi = 300, width = 11.5, height = 8)
#ggsave("LA_map_RPKM_by_genome.pdf", dpi = 300, width = 11.5, height = 8)
ggsave("DZ_map_RPKM_by_genome.pdf", dpi = 300, width = 11.5, height = 8)
...
# Organize by increasing salinity for SFBay samples 
sub_data$sample <- factor(sub_data$sample , levels=c("C33_657D", "C33_649D", "C33_3D", "C33_9D", "C33_13D", "C33_34D", "C33_18D", "C33_27D"))
...
ggsave("genomes_by_rpkm_barplot_SFBay.pdf", dpi = 300, width = 11.5, height = 8)
...
ggplot(data=datamelt, aes(x=RPKM)) +
  ...
  labs(x="RPKM", y="Number") +
  facet_wrap(~ genome) + 
  theme_classic()

ggplot() +
  geom_boxplot(data=datamelt, aes(y=RPKM, x=genome), outlier.shape = 1) +
  labs(x="Genome", y="RPKM") +
  theme_classic() +
  theme(axis.text.x=element_text(angle=90, size=8))

#ggsave("genomes_by_rpkm_boxplot.pdf", dpi = 300, width = 11.5, height = 8)
ggsave("genomes_by_rpkm_boxplot_coastal.pdf", dpi = 300, width = 11.5, height = 8)
...
ggsave("genomes_by_rpkm_boxplot_SFBay.pdf", dpi = 300, width = 11.5, height = 8)
...
ggsave("Temp_v_RPKM.pdf", dpi = 300, width = 11.5, height = 8)
...
#ggsave("Sal_v_RPKM.pdf", dpi = 300, width = 11.5, height = 8)
ggsave("coastal_Sal_v_RPKM.pdf", dpi = 300, width = 11.5, height = 8)
...
ggsave("Depth_v_RPKM_nonlinear.pdf", dpi = 300, width = 11.5, height = 8)
...
ggsave("Depth_v_RPKM_linear.pdf", dpi = 300, width = 11.5, height = 8)
...
ggsave("Lat_v_RPKM.pdf", dpi = 300, width = 11.5, height = 8)
...
ggsave("numsites_v_lat.pdf", dpi = 300, width = 11.5, height = 8)
...
ggsave("densites_v_lat.pdf", dpi = 300, width = 11.5, height = 8)
